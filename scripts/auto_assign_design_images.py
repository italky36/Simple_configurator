    ...
    for color_entry in color_entries:
        ...
        for insert_entry in insert_entries:
            ...
            def normalize_color_key(color_name: str) -> str:
                """Нормализует название цвета к формату, ожидаемому фронтендом (с буквой ё где нужно)."""
                normalized = color_name.lower()
                # Маппинг всех цветов к формату фронтенда
                # FRAME_COLORS: ["белый", "чёрный"]
                # INSERT_COLORS: ["жёлтый", "зелёный", "красный", "серый", "синий", "фиолетовый"]
                color_map = {
                    "белый": "белый",
                    "черный": "чёрный",
                    "желтый": "жёлтый",
